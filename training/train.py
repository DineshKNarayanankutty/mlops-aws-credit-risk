...
Reads data from S3 (via SageMaker channel), trains a RandomForest,
uploads model.pkl and metrics.json to S3.
"""
import argparse
import json
import logging
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--output-data-dir", type=str, default=os.getenv("SM_OUTPUT_DATA_DIR", "./artifacts/output"))
...
def upload_artifacts_to_s3(model_path: Path, metrics_path: Path, model_s3_uri: str) -> None:
    ...
    logger.info('"Uploaded model to s3://%s/%s"', bucket, model_key)
...
def main() -> None:
    ...
    logger.info('"Starting training with n_samples=%d n_features=%d"', args.n_samples, args.n_features)
    ...
    # TODO (production): Replace make_classification with real credit data loaded from S3
